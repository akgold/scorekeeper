...
/***/ function(module, exports, __webpack_require__) {
	...
	function draw(err, data) {
	  if (err) {
	    ...
	    (function () {
	      data = _underscore2.default.sortBy(data, function (d) {
	        return -d.points;
	      });

	      var fontScale = d3.scaleLinear().range([1, 3]).domain(d3.extent(_underscore2.default.pluck(data, 'points')));
	      var fontMap = function fontMap(d) {
	        return fontScale(d.points) + 'em';
	      };
	      var rows = d3.select('tbody').selectAll('tr').data(data, function (d) {
	        return d.name;
	      });

	      rows.enter().append('tr').style('font-size', fontMap).each(function (d) {
	      ...
	      rows.style('font-size', fontMap).each(function (d) {
